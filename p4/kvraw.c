...
kvraw_open(const char *pathname)
{
	...
	/* the storage is empty, perform initialization */
	if (size == 0)
	{
		off = 0;
	...
	{
		struct meta meta; /* metadata: the first key-value pair */
		kvraw->size = size;

		/* read metadata from offset 0 (the first entry) */
		...
		/* key or value lengths from the metadata are zero */
