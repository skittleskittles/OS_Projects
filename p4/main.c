...
heavy_rewrite(void)
{
	...
	for (i = 0; i < n; ++i)
	{
		...
		if (kvdb_update(kvdb, KEY, SLEN(KEY), val, SLEN(val)) ||
			(lookup_res = kvdb_lookup(kvdb, KEY, SLEN(KEY), val_, &val_len_)) ||
			(SLEN(val) != val_len_) ||
			memcmp(val, val_, val_len_) ||
			(1 != kvdb_size(kvdb)) ||
			(i != kvdb_waste(kvdb)))
...
/**
 * store_restore_file:
 * 	insert KEY-VAL to the file system,
 * 	if reopen, KEY-VAL should still exist in the file system
 */
static int store_restore_file(void)
{
	const char *const KEY = "KEY";
...
int main(int argc, char *argv[])
{
	...
	{
		TEST(store_restore_file, "store_restore_file");
	}
	else
	{
